...
class MMC_MEB_Source_Service {
    ...
    public static function info() {
        ...
        $out['metric_count'] = (int) $wpdb->get_var( "SELECT COUNT(DISTINCT CONCAT(province_name,'|',metric_key)) FROM $table WHERE $where" );
        $out['province_count'] = (int) $wpdb->get_var( "SELECT COUNT(DISTINCT province_name) FROM $table WHERE $where" );
        $out['school_total'] = (int) round( (float) $wpdb->get_var( "SELECT SUM(metric_value) FROM $table WHERE $where AND metric_key='school_count'" ) );
        $out['student_total'] = (int) round( (float) $wpdb->get_var( "SELECT SUM(metric_value) FROM $table WHERE $where AND metric_key='student_count'" ) );
        ...
        if ( ! $handle ) {
            $response = wp_remote_get( self::DATASET_URL, array(
                ...
                'headers'     => array(
                    ...
                    'User-Agent'  => 'MadagaskarManagementCenter/' . MMC_VERSION,
                ),
            ) );

            if ( is_wp_error( $response ) ) {
                return new WP_Error( 'mmc_meb_download', 'MEB veri dosyası indirilemedi: ' . $response->get_error_message() );
            }

            $code = (int) wp_remote_retrieve_response_code( $response );
            $body = (string) wp_remote_retrieve_body( $response );
            if ( 200 !== $code || '' === trim( $body ) ) {
                return new WP_Error( 'mmc_meb_download', 'MEB veri dosyası indirilemedi (HTTP ' . $code . ').' );
            }

            $handle = fopen( 'php://temp', 'r+' );
            if ( ! $handle ) {
                return new WP_Error( 'mmc_meb_read', 'MEB veri dosyası için geçici akış açılamadı.' );
            }
            fwrite( $handle, $body );
            rewind( $handle );
        }
