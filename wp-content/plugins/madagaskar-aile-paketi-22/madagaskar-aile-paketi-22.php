...
final class MDG_Family_Package_22_V1 {
    ...
    private static function price_for_event( $event_id ) {
        ...
        return ( $event_id && ! empty( $settings['event_prices'][ $event_id ] ) ) ? round( (float) $settings['event_prices'][ $event_id ], 2 ) : round( (float) $settings['price'], 2 );
    }
    ...
    public static function render_admin_page() {
        ...
        if ( ! $events ) {
            ...
            foreach ( $events as $event ) {
                ...
                echo '<td><input name="event_prices[' . $event_id . ']" type="number" min="1" step="0.01" style="width:110px" value="' . esc_attr( number_format( $event_price, 2, '.', '' ) ) . '"> TL</td>';
