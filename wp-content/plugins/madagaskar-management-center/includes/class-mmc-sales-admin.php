...
class MMC_Sales_Admin {
    ...
    private function render($program){
        ...
        <?php $source_id=absint($_GET['legacy_preview']??0); $source_preview=$source_id?MMC_Sales_Service::legacy_import_preview($event->id,$source_id):null; ?>
        <form method="get" action="<?php echo esc_url(admin_url('admin.php')); ?>" class="mmc-inline-form"><input type="hidden" name="page" value="mmc-sales"><input type="hidden" name="program_id" value="<?php echo (int)$program->id; ?>"><label>Satıştaki MDG Event ID <input type="number" min="1" name="legacy_preview" value="<?php echo esc_attr($source_id?:''); ?>" required></label><button class="button">Bağlantıyı Önizle</button></form>
        <?php if($source_preview): ?><h3><?php echo esc_html($source_preview['legacy_title'].' · #'.$source_id.' · '.$source_preview['legacy_status']); ?></h3>
        <?php if(!$source_preview['ready']): ?><div class="notice notice-warning inline"><p><?php echo esc_html(implode(' · ',$source_preview['errors'])); ?></p></div><?php endif; ?>
        <table class="widefat striped"><thead><tr><th>Seans</th><th>Bilet</th><th>WC ürün</th><th>WC varyasyon</th><th>Tickera</th></tr></thead><tbody><?php foreach($source_preview['rows'] as $row): ?><tr><td><?php echo esc_html($row['time']); ?></td><td><?php echo esc_html($row['ticket_name']); ?></td><td><?php echo (int)$row['product_id']; ?></td><td><?php echo (int)$row['variation_id']; ?></td><td><?php echo (int)$row['tickera_event_id']; ?></td></tr><?php endforeach; ?></tbody></table>
        <?php if($source_preview['ready']&&(current_user_can('mmc_manage_sales')||current_user_can('mmc_manage_programs'))): ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="mmc_import_legacy_sales_mapping"><input type="hidden" name="event_id" value="<?php echo (int)$event->id; ?>"><input type="hidden" name="program_id" value="<?php echo (int)$program->id; ?>"><input type="hidden" name="legacy_event_id" value="<?php echo (int)$source_id; ?>"><?php wp_nonce_field('mmc_import_legacy_sales_mapping_'.$event->id,'mmc_nonce'); ?><p><button class="button button-primary">Doğrulanan <?php echo count($source_preview['rows']); ?> Satış Bağını Kur</button></p></form><?php endif; ?>
