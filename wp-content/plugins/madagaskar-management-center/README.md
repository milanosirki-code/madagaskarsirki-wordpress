...
- `mdg-dashboard`, `madagaskar-v4`, `mad-okul` ve bilinen `madagaskar-etkinlik-yayinla` üst menüleri ilgili eklenti aktifse gizlenir.
- Eski sayfa URL/callback kayıtları korunur; doğrudan `admin.php?page=...` erişimi çalışmaya devam eder.
- V4 ekranları işlevlerine göre Bilet Yönetimi, Satış & Müşteri ve Sistem & Yetkiler merkezlerine dağıtılır.
- Okul Tanıtım ekranları Okul Tanıtım & Saha merkezinde kalır.
- MMC devre dışı bırakılırsa legacy üst menüler kendi eklentileri tarafından yeniden görünür.
- `mmc_view_dashboard` yetkisi olmayan legacy kullanıcıların mevcut navigasyonu değiştirilmez.
- `MMC_Navigation_Admin::navigation_health()` ile bilinen legacy üst menüler salt-okunur kontrol edilebilir.
